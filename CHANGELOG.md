...
- 모바일 첫 화면에 용도가 보이는 56px 높이의 세 앱 빠른 실행을 추가하고, 중복되던 상단 앱 링크를 대체했다.
- 고정된 시각과 `LIVE` 표현을 알림 예시로 바꿔 실제 실시간 상태로 오인되지 않게 했다.
- 빠른 실행 안내 대비와 보조 링크 터치 영역을 보완하고, 375·390·414px 모바일과 데스크톱 화면을 다시 검증했다.
