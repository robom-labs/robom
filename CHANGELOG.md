...
### 추가 (패밀리 개편 1/4 — robom.kr)

- 커스텀 "봄" 워드마크(brand/bom-mark, PR #33 스펙)를 사이트 브랜드 락업에 적용 — 본사 3색(코랄·청록·금), sr-only "봄" 병기, 정식 이름 불변.
- 세 앱 카드에 이중 CTA: [웹으로 열기] + 스토어 등록 전이라 [앱 출시 준비 중] 표시(PLAY_URLS 상수, null이면 스토어 링크 미노출 — 죽은 링크 방지).
- 패밀리 푸터: 계열사 앱 링크 · 제공: 로봄 · 버전(루트 VERSION 빌드타임 주입) · 문의·광고 hello.robom@gmail.com · 정책 링크.
- **/privacy · /terms 정책 페이지 신설**(스토어 등록용 공개 정본) + 정적 프리렌더를 3라우트로 확장.
