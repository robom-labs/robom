...
- 종료된 과거 앱의 도메인 문구와 링크가 공개 홈페이지에 다시 나타나지 않도록 4개 운영 앱 기준 회귀 검사를 추가했다.
- 청약봄 검색 설명에 남아 있던 종료 앱 연상 문구를 현재 기능 중심 표현으로 교체했다.
- Windows에서 생성물·호환성 검사가 드라이브 경로를 중복하거나 줄바꿈 형식을 변경으로 오인해 빌드가 중단되던 문제를 수정했다.
- Windows에서 Unix 전용 환경변수 문법 때문에 사이트 빌드 명령이 시작되지 않던 문제를 수정했다.
