...
- 내부 로봄 본부 오피스를 6개 층·24개 대기업형 공간과 회장 집무실·이사회실·제품 랩·로비·데이터센터·시설창고를 갖춘 아이소메트릭 게임으로 확장했다.
- 직원 20명을 남성 4명·여성 16명으로 구성하고 헤어·의상·피부톤·안경·헤드셋 조합을 캐릭터별로 분리했다.
- 직원 상세 패널, 방 클릭 확대, 모바일 핀치 줌, 상태 전이 토스트와 실시간 이벤트 기반 회의·막힘·배포 표시를 추가했다.
- 목적·담당·동선이 있는 방문 연출 캐릭터 11명을 추가하고 1층 로비를 가장 활발한 층으로 구성했다.
- 업무 지시에 담당 직원을 지정하고 오피스 직원 상세에서 지시 대기와 실제 실행 업무를 구분해 확인할 수 있게 했다.

### Changed

- 실제 작업 이벤트가 없을 때는 대기·순찰·휴식·식사만 수행하고, 업무·회의·배포 상태는 실제 snapshot 근거가 있을 때만 표시하도록 고정했다.
- 오피스 정적 바닥 레이어 캐시와 reduced-motion 대응을 추가하고 320px부터 태블릿까지 HUD와 상세 패널을 최적화했다.
- 회장 메뉴를 13개 핵심 항목과 5개 바로가기로 축소하고, 각 층을 두 업무 축·네 공간·실제 운영 신호 중심으로 다시 구성했다.
- 설치된 로컬 본부가 새 화면 자산을 오프라인에서도 받도록 Company OS 셸 캐시를 `v2.1.0`으로 갱신했다.

### Fixed

- 회의 중 막힘·승인 대기·배포 상태가 회의 표시로 덮이는 문제와 상태 전이 토스트에 영문 상태명이 노출되는 문제를 수정했다.

## [2.2.1] - 2026-07-18

### Added

- 앱 소개·설치 경로에 실제 registry 버전 기반 `SoftwareApplication`과 `BreadcrumbList` 구조화 데이터를 추가했다.
- 공개 검색 경로의 title·description·canonical 유일성, 네이버 인증, JSON-LD와 sitemap 계약을 검증하는 회귀 테스트를 추가했다.

### Changed

- 지원·약관·개인정보·라이선스 페이지에 실제 내용별 고유 검색 설명을 제공한다.
- sitemap을 registry 생성물에서 결정적으로 만들고 `/terms`·`/licenses`를 검색 경로에 포함한다.

### Fixed

- 러닝봄 최신 운영 SHA와 패밀리 호환성 정본이 달라 홈페이지 Pages 배포가 차단되던 문제를 수정했다.
...
- 설치 허브 `/get/*`·`/apps/*` 브레드크럼 링크의 터치 영역을 44×44px 이상으로 확장했다 (2회차 모바일 UX 감사).
- 분석 이벤트의 `app_version` 하드코딩을 `SITE_VERSION` 참조로 바꿔 버전 인상 시 드리프트를 제거했다.

### Changed

- 등록부 정본을 homebom 0.14.2·runningbom 0.17.3으로 올리고 family 생성물을 재생성했다.
...
- 앱별 밝은 브랜드색과 흰 글자 조합을 고대비 동작색으로 분리하고 가로 기능 목록을 키보드로 탐색할 수 있게 했다.
- GitHub raw 버전 정본의 CDN 지연 때문에 최신 앱 배포를 이전 버전으로 오판하던 registry 검사를 no-store 조회로 바꿨다.
- 홈페이지 서비스워커와 업데이트 등록을 추가하고, 운영 axe 감사가 앱별 내부 새로고침과 충돌하지 않게 화면마다 격리했다.
- Pages E2E가 자식 서버 프로세스를 남기지 않게 종료를 보장하고 새 배포가 멈춘 실행을 취소할 수 있게 했다.
...
- `ops/registry/apps.yml`을 앱 버전·URL·스토어 상태의 단일 정본으로 삼고 홈페이지 데이터·설치 랜딩·QR을 생성하도록 전환했다.
- 실제 스토어 출시 전에는 가짜 배지를 표시하지 않고 플랫폼별 PWA 설치와 웹 체험을 제공한다.
- 분석과 광고는 기본 OFF로 유지하고, 금지 개인정보 검사와 비공개 성장 보고 파이프라인만 준비했다.

## [1.8.0] - 2026-07-14

### Changed

- Google 검색에서 로봄의 용도와 세 앱이 더 분명히 보이도록 제목·설명·첫 화면 카피를 정리했다.
- 검색 결과용 고대비 루트 favicon과 PWA 아이콘을 고정 URL로 제공하고 사이트명·조직 structured data를 강화했다.
- 야외봄·청약봄·러닝봄 소개 페이지의 고유 메타데이터와 breadcrumb를 추가했다.
- 중앙 레지스트리와 소개 페이지의 앱 버전을 야외봄 0.22.0, 청약봄 0.10.0, 러닝봄 0.15.0으로 동기화했다.

## [1.7.5] - 2026-07-13

### Changed

- 야외봄 0.19.0, 청약봄 0.7.0, 러닝봄 0.13.0의 패밀리 공통 하단 내비게이션과 설정 정보 구조를 중앙 레지스트리·홈페이지·상태 장부에 동기화했다.
- 0.12.0의 접수 중 우선 화면과 단일 공식 접수처 CTA가 유지되는지 재검증하고 GitHub Pages와 Vercel 보조 배포를 `9941247` 기준으로 맞췄다.

## [1.7.4] - 2026-07-13

### Changed

- 러닝봄 0.12.0의 접수 중 우선 화면, 마감 임박 바로가기, 의미가 포함된 카운트다운과 단일 공식 접수처 CTA를 중앙 레지스트리·홈페이지·상태 장부에 동기화했다.
- GitHub Pages와 `https://runningbom.vercel.app/` 보조 운영 배포를 같은 빌드 `d402783`로 확인했다.
...
- 세 앱의 검증·배포 완료 버전을 야외봄 0.18.0, 청약봄 0.6.0, 러닝봄 0.11.2로 홈페이지·운영 레지스트리·상태 장부에 동기화했다.
- 야외봄 준비 탭 단순화, 청약봄 일정·운영 데이터 보강, 러닝봄 커스텀 `봄` 트레이드마크 복원 상태를 중앙 운영 기록에 반영했다.

## [1.7.1] - 2026-07-13

### Changed

- 야외봄 공식 운영 주소를 `https://outbom.robom.kr`로 전환하고 홈페이지·운영 레지스트리·상태 장부를 0.17.1에 맞췄다.
- 기존 `https://runningcall.vercel.app`은 호환 주소로 기록해 설치 사용자와 기존 링크를 유지한다.

## [1.7.0] - 2026-07-13

### Changed

- 모바일 320×568 첫 화면에서 러닝봄 타일이 고정 하단 메뉴에 가리던 높이 계산을 바로잡았다.
- 세 앱 운영 버전을 야외봄 0.17.0, 청약봄 0.5.0, 러닝봄 0.11.1로 동기화했다.
- 문의·광고·제휴 메일 제목 생성 경로를 하나로 합치고 사용하지 않던 문의 종류 분기를 제거했다.
- GitHub Pages 미리보기의 내부 링크와 PWA manifest를 `/robom/` 하위 경로에서도 동작하도록 상대 경로화했다.
- 앱 레지스트리 변경도 홈페이지 배포와 즉시 drift 검사에 포함했다.

## [1.6.0] - 2026-07-13

### Changed

- 모바일 첫 화면에서 제목 직후 세 앱을 보여주고, 소개 페이지를 거치지 않고 실제 웹 앱을 새 창으로 바로 열게 했다.
- 야외봄 0.16.0, 청약봄 0.4.0, 러닝봄 0.10.0의 실제 운영 버전과 핵심 기능 문구를 사이트·레지스트리·상태 장부에 동기화했다.
- 청약봄의 과장된 정정·취소 표시 문구를 실제 제공 기능인 일반·무순위 일정과 발표·계약 캘린더로 교체했다.
- 320·390px 모바일에서 앱 바로 열기, 하단 내비, 가로 넘침과 터치 영역을 검증하는 구조로 첫 화면 밀도를 낮췄다.

## [1.5.0] - 2026-07-13

### Changed

- 충돌 난 기존 최적화 변경에서 OG 1200×630 압축, PWA manifest, robots·sitemap, JSON-LD와 본문 바로가기를 최신 main에 맞춰 선별 반영했다.
- 로컬 SVG에 불필요한 이미지 런타임을 쓰지 않도록 일반 이미지 요소로 바꾸고 러닝봄의 오래된 “이번 주 접수” 문구를 현재 월간 캘린더에 맞췄다.
- 운영 레지스트리와 사이트 표시 버전을 야외봄 0.15.2, 청약봄 0.3.3, 러닝봄 0.9.6으로 동기화했다.
- 세 앱의 실제 운영 버전을 본사 레지스트리와 홈페이지에 맞추고, 원본 package.json을 매일 비교하는 drift 검사를 추가했다.
- 야외봄·청약봄·러닝봄 전용 개인정보처리방침과 페이지별 canonical 주소를 추가했다.
- 실제로 사용하지 않는 D1 예제·ChatGPT 인증 템플릿과 워커 DB 바인딩을 제거해 홈페이지 배포 범위를 명확히 했다.
- robom.kr이 운영체제 다크 모드에 따라 칙칙해지지 않도록 밝고 따뜻한 봄 테마를 고정했다.
- 외부 앱 웹 이용은 새 창으로 열고, 고객 지원과 광고·제휴 문의는 하나의 이메일 창구로 통합했다.
- 로봄·야외봄·청약봄·러닝봄을 같은 둥근 타일과 봄 점으로 묶은 대표 아이콘 4종을 추가하고, 홈페이지 파비콘·헤더·앱 카드에 적용했다.
