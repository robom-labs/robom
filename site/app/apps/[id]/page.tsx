  ...
  return (
    <PageShell current="apps">
      <section className={`product-page ${app.tone}`}>
        ...
        <div className="product-hero"><div className="product-copy"><p className="card-eyebrow">{app.eyebrow}</p><h1>{app.heroTitle}</h1><p>{app.heroBody}</p><div className="product-actions"><a className="button primary" href={app.webUrl} target="_blank" rel="noopener noreferrer">{app.accessLabel} <span aria-hidden="true">↗</span></a><a className="button secondary" href={contactHref("general", app.name)}>문의하기</a></div></div><div className="product-score" aria-label={`${app.name} 버전`}><small>현재 버전</small><strong>v{app.version}</strong><span>WEB PWA</span></div></div>
