  ...
  return (
    <PageShell current="apps">
      <section className={`product-page ${app.tone}`}>
        ...
        <div className="product-hero"><div className="product-copy"><p className="card-eyebrow">{app.eyebrow}</p><h1>{app.heroTitle}</h1><p>{app.heroBody}</p><div className="product-actions"><Link className="button primary" href={app.installPath} prefetch={false}>{app.accessLabel} <span aria-hidden="true">→</span></Link><a className="button secondary" href="#app-preview">화면 미리보기 <span aria-hidden="true">↓</span></a></div><a className="product-contact" href={contactHref(app.name)}>문의·의견 보내기</a></div><div className="product-score" aria-label={`${app.name} 버전`}><small>현재 버전</small><strong>v{app.version}</strong><span>{app.mobileStatus.toUpperCase()}</span></div></div>
        ...
        {/* 화면 미리보기: 보기만 가능한 실제 화면(조작 불가). 사용은 QR 설치로 안내한다. */}
        <section className="app-preview" id="app-preview" aria-labelledby="app-preview-title">
          <div><p className="eyebrow"><span aria-hidden="true" /> PREVIEW</p><h2 id="app-preview-title">{app.name} 화면 미리보기</h2><p className="preview-note">실제 운영 화면을 보여드리는 미리보기예요. 조작은 되지 않아요 — 사용하시려면 QR로 휴대폰에 설치해 주세요.</p><Link className="button primary" href={app.installPath} prefetch={false}>QR로 설치하기 <span aria-hidden="true">→</span></Link></div>
          <div className="phone-frame" aria-hidden="true"><iframe src={app.webUrl} title={`${app.name} 미리보기`} loading="lazy" tabIndex={-1} /></div>
        </section>
