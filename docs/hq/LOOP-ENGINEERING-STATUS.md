...
- **24차(CI 판정 두 화면 일치 — hq-v3.3.19)** → 이월 warn 드롭 처리. 배포 결론이 success도 hard-fail(failure·timed_out·startup_failure)도 아닌 종료(cancelled·neutral·action_required·stale)를 `ciResult`가 PASS로 위장해, build-snapshot이 'warn'으로 보는 것과 어긋나고 무-probe 앱에서 조용히 드롭됐다. → success만 PASS, hard-fail은 FAIL, 그 외 종료는 **DEGRADED-info**로 노출(초록 위장 없음·사건/결재 스팸 없음, 두 화면 일치). 테스트 추가. **이월 중간건 전부 소진**. 미감사 잔여: events.mjs·autostart.mjs(다음 라운드).
