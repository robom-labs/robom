...
test("결정론적 health 엔진이 review에 연결되고 incident만 중복 없이 결재로 상신된다", () => {
  ...
  // Loop 종료 정직화: (H2) orphan_pruned는 회복이 아니라 retired → PASS 위장 금지, (H1) 코드변경 Loop는 회복돼도 회귀 먼저 검사
  assert.match(server, /UNVERIFIED_RETIRED/);
  assert.match(server, /recHasRegression/);
  assert.match(server, /regression-on-recovery/);
  // 자기검증 후속: 승격 결재 중복 방지는 열린 Loop 유무로(existingKeys는 승인된 결재를 놓침), 회복 시 #escalated 결재도 함께 닫힘, loop 클래스는 authorityClass로 판정
  assert.match(server, /if \(findLoopByContract\(r\.contractId\)\) continue/);
  assert.match(server, /replace\(\/#escalated\$\/, ""\)/);
  assert.match(server, /loop\.authorityClass !== "self_heal"/);
...
test("실제 스냅샷은 완성된 임시 파일을 원자적으로 교체한다", () => {
  ...
  // M1: 임베드 runs는 크기 제한하되 비종료(대기·승인) 전부 보존 — latest.json 무한 증가 방지, 장기 대기 유실 금지
  assert.match(builder, /const embeddedRuns/);
  assert.match(builder, /runs: embeddedRuns/);
  assert.match(builder, /!TERMINAL_RUN_STATUS\.has\(r\.status\)/); // 비종료 전부 보존
});
