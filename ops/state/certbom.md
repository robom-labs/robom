...
- 운영 main: `a1dcdf2a874a15d1dd59519bceb00e7116cbb7dc`.
- PWA 캐시: `certbom-0.8.3-a1dcdf2`.
- Google Play Alpha: versionCode 9, `0.8.3`, 테스터 제공 상태.
- 핵심 기능: 시험 104개와 확정 일정 보유 시험 70개 탐색, 현재 접수 우선 정렬, 약칭 검색, 상태·분야 필터, 질문 5개 기반 추천 3+7, 관심 시험·준비 체크·다가오는 순서의 내 일정, 관심 시험 전체 일정 ICS 내보내기, 기기 데이터 JSON 백업·복원, Google Calendar·공유, PWA 오프라인 셸.
- 데이터 원칙: 공식 출처 8곳의 일정 내용 검토 시각과 자동 연결 점검 시각을 분리해 표시하며 확인되지 않은 시험 시각이나 접수일을 추정하지 않는다. 2026-08-08 연결 점검은 8곳 모두 정상이며, 일정 내용 검토 시각은 2026-07-24로 별도 유지한다. Q-Net 공공데이터 운영 키 등록 전에는 API 연결로 표시하지 않는다.
