...
## 방금 한 일 (최근)
- 2026-07-10 daily#3: 세 앱 출시 준비 PR(#8/#5/#4)이 모두 머지·자동배포된 것을 확인(이전 state의 "대기"는 stale). 각 앱에서 작고 안전한 변경 1개씩 추가 진행 후 draft PR 생성: zoopzoopcall SW 셸 precache(#9), runningcall lib/insights.ts 테스트 16개(#6), pushrun 검색 Enter 키 필터(#5). 세 앱 모두 typecheck/test/build(또는 정적 검증) 통과 확인 후에만 PR 생성. ESLint 설치(lockfile 위험)·races.json 최신화(버그 아님 확인)·app.js 모듈분리는 재조사 후 보류.
...
- [ ] 앱 PR 머지: zoopzoopcall #9, runningcall #6, pushrun #5 (모두 오늘 daily 소규모 변경, 이전 출시 준비 PR은 이미 머지 완료)
...
- [ ] 앱 코드 개선(각 원본 저장소): zoopzoopcall dedup(R3/R10)·폰트 self-host, runningcall ESLint 재검토·page.tsx 분해, pushrun HTTP→HTTPS·races.json 최신화
