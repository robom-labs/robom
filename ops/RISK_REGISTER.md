...
| R9 | runningcall /api/* 무인증 → Kakao 유료키 소진 | 비용/쿼터 고갈 | 레이트리밋·오리진 제한(앱 저장소 작업) | 완화(PR#2 draft, 머지 대기) |
| R10 | 3개 앱 공통로직 3벌 중복(KST·알람·알림) | 표류·버그 | 공유 core 패키지화 | 열림(P1) |
| R11 | 3개 앱 모두 "알림이 조용히 안 감" HIGH 버그(2차 감사) | 앱 핵심가치 실패 | 각 원본 저장소에서 수정(ROADMAP P0-신규, handoff) | 열림(P0) |
