# base path 보호 플레이북 — "/homebom/" 는 절대 안 깨진다

## 왜
`apps/web/vite.config.ts`의 `base: "/homebom/"`는 배포 URL
`https://runnerpyrri-lgtm.github.io/homebom/`와 1:1이다. 바뀌면 기존 사용자 링크가 전부 깨진다.
...
4. **배포 스모크**: 릴리스 전 `pnpm build` 후 `dist/index.html`에 `/homebom/` 자산 경로 확인.
