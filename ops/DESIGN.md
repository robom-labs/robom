# 로봄 — 1인 지주회사 자동 운영 시스템 설계서

> 2026-07-11 D21이 현재 구조의 정본이다. 앱 소스는 `robom`에 복사하지 않고
> `robom-labs/outbom`, `robom-labs/homebom`, `robom-labs/runningbom`에서 독립 관리한다.
> 아래 내용은 초기 설계 근거와 의사결정 이력을 보존한 기록이다.
