...
- 2026-07-14: 0.24.0. 오늘 탭 히어로에 현재 시각 마커를 추가하고 PC·모바일 배치를 최적화했다.
- 2026-07-14: 0.22.1. 전수 감사 수정 — SW 첫방문 리로드·클라이언트 타임아웃·내일 일몰 수정, 바텀시트 접근성 통일, 죽은 코드 정리.
- 2026-07-13: 0.20.0. 위치를 `고척동 · 서울 구로구` 한 줄 bar로 정리하고 추천 탭을 활동별 결론 카드로 재설계했다. 강수 상세를 전용 24시간 화면으로 바꾸고 dead CSS 테마를 제거했다(gzip -48%).
- 2026-07-13: 0.19.0. 하단 내비를 로봄 패밀리 공통 규격으로 통일하고 설정 IA를 맞췄다.
- 2026-07-13: 0.18.1. 공식 운영 화면을 GitHub Pages `/outbom/`으로 통일하고 기존 Vercel 배포는 API와 설치 사용자 호환용으로 유지했다. 로봄 패밀리 공통 Pretendard 워드마크 규격을 적용했다.
- 2026-07-13: 0.18.0. 랜덤 카드·큰 회전 로딩·준비물 모달을 제거하고 준비 탭에 전체 준비 항목을 바로 표시했다.
- 2026-07-13: 0.17.1. `outbom.robom.kr`을 공식 주소로 연결하고 기존 Vercel 주소를 호환용으로 유지했다.
