...
- 2026-08-08: 0.21.0. 실제 월별 7열 캘린더, 한국시간 접수 마감 카운트다운, 마감 임박 바로가기와 48px 터치 셀을 추가했다. 5K·10K 등 여러 종목은 한 대회 카드 안에 유지하고 대회·카드·캘린더 날짜 수가 종목 수 때문에 늘어나지 않는 회귀 검사를 통과했다.
- 2026-08-08: 0.20.0. 5K·10K·하프·풀을 거리별 대회로 중복 표시하지 않고 대회 한 개·카드 한 개 안에 종목을 통합했다. 접수 상태·관심 필터·출처·검증 시각·알림 취소를 보강하고 6시간 자동 갱신·watchdog·이상 데이터 차단·마지막 정상본·400일 장애 주입을 검증한 뒤 Android 0.20.0(14) AAB를 생성했다.
