...
- 2026-07-16: 0.13.0. 청약홈 호출 제한과 모델 cold cache에 검증 스냅샷을 보존하고, 공식 PDF 파서를 보수적으로 강화했다. 문서 동기화가 개별 PDF 오류에 막히지 않도록 격리·재시도하며, 1,000행 초과 캐시 조회를 페이지네이션했다. 모바일 첫 공고 위치·달력 접근성·상세 알림 딥링크·상세 로딩과 PWA 캐시 범위를 고치고 Pages·Supabase 운영을 재검증했다.
- 2026-07-14: 0.12.0. 공식 공고문 동기화·필드별 출처와 충돌 처리·영구 검증 스냅샷·서버/클라이언트 런타임 계약을 적용하고, 마감·취소 공고와 반복 fallback 문구를 제거했다. Pages 운영 API 번들 검증과 PWA 캐시 zzc-v23을 반영했다.
- 2026-07-14: 0.10.1. 전수 감사 수정 — 설정 화면 크래시(알림 미지원 브라우저)·아젠다 404·달력 마감일 클릭 수정, 탭 복귀 자동 갱신.
- 2026-07-13: 0.7.1. 청약홈 공고 원문 링크가 종종 404를 내던 문제를 고쳤다. 공공데이터 API가 PBLANC_URL의 `&`를 `&amp;`로 이스케이프해 pblancNo 파라미터가 깨지던 것을 URL 정규화에서 되돌리고, 저장된 캐시 링크도 렌더 시 복구한다. 캐시 zzc-v15.
- 2026-07-13: 0.7.0. 설정 화면을 패밀리 공통 IA로 재배치하고 하단 내비 규격을 맞췄다.
- 2026-07-13: 0.6.0. 커스텀 `봄` 마크, 일정 고정 순서·개별 행, 공고 모델·지도 fallback과 운영 데이터 검증을 반영했다.
